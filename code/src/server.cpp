...
index_got_responded:;
	...
	bool TryFileRequest(const string &session_token,const string &receiver_name,const string &file_size,string &response)
	{
		...
		int n;
		try{n=stoi(file_size);}
		catch(...){return response="file_size isn't a number",false;}
...
bool ProcessMessage(const string &msg,string &response)
{
	...
	{
		if(args.size()!=2)return response="check_file_request expect 2 params, got "+to_string(args.size()),false;
	...
	{
		if(args.size()!=3)return response="receive_file expect 3 params, got "+to_string(args.size()),false;
